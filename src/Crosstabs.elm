...
import XB2.Share.Data.Platform2 exposing (AudienceId)
...
type alias Configure msg =
    ...
    , editAudienceInP2 : AudienceId -> msg
...
type Msg
    ...
    | SaveAsAudience Modal.SaveAsItem Caption AudienceExpression
    ...
    case action of
        ...
        ShareProjectWithLink ->
            (if project.shared == XBData.SharedByLink then
                ...
                |> Cmd.add (Analytics.trackEvent flags Place.CrosstabBuilder <| Analytics.CopyLink { projectId = project.id, projectName = project.name })
...
update config zone flags maybeUrl route p2Store msg model =
    (case msg of
        ...
        DetailMsg dMsg ->
            Cmd.pure model
                |> Glue.Lazy.updateWith detail
                    (Detail.update config.detailConfig flags model.xbStore p2Store dMsg)

        ListMsg lMsg ->
            Cmd.pure model
                |> Glue.Lazy.updateWith list
                    (XBList.update config.listConfig flags zone model.xbStore p2Store lMsg)

        AnalyticsEvent place event ->
            Cmd.with (Analytics.trackEvent flags place event) model

        ModalMsg mMsg ->
            model.modal
                |> Maybe.map
                    (Modal.update config.modalConfig flags model.xbStore mMsg
        ...
        CreateXBProject ->
            model
                ...
                |> Cmd.add (Analytics.trackEvent flags Place.CrosstabBuilderList ProjectCreationStarted)
        ...
        SaveAsAudience saveAsItem caption expression ->
            let
                ...
                newExpression =
                    case expression of
                        Leaf data ->
                            Node Expression.Or <| NonemptyList.singleton <| Leaf data
        ...
        BrowserModalItemToggled item selectedItems ->
            ...
            model
                ...
                |> Cmd.addTrigger (config.msg <| CheckIncompatibilitiesAfterToggle newSelectedItems)
