...
import XB2.Views.Modal as Modal exposing (AffixBaseGroupData, Modal)
...
itemsFromXBItems grouping xbItems =
    let
        constructWith : (NonEmpty AudienceExpression -> AudienceExpression) -> NonEmpty (Random.Seed -> ( AudienceItem, Random.Seed ))
...
openSavedProject :
    ...
    -> Model
    -> ( Model, Cmd msg )
openSavedProject config flags fullProject maybeLoadedCrosstab p2Store maybeSettings model =
    let
        clearModel =
            initWorkspace
                flags.can
    ...
    initialiasedCrosstabAndCmds
        ...
        |> Glue.updateWith Glue.id (fetchWavesAndLocations config flags p2Store { updateCurrentHistory = False })
...
type EditMsg
    ...
    | SetGroupTitle
        ...
        , expression : Maybe AudienceExpression -- avg: Nothing, non-avg: Just
        }
    | SetGroupTitles
        (NonEmpty
            { direction : Direction
    ...
    | Undo
    | Redo
    | AudienceFromSelectionCreated AudienceExpression AudienceItem
...
maybeTrack : Flags -> Maybe Event -> Cmd msg
maybeTrack flags maybeEvent =
    maybeEvent
        |> Maybe.map (track flags)
        |> Maybe.withDefault Cmd.none


track : Flags -> Event -> Cmd msg
track flags event =
    Analytics.trackEvent flags Place.CrosstabBuilder event


trackMany : Flags -> List Event -> Cmd msg
trackMany flags events =
    Analytics.trackEvents flags Place.CrosstabBuilder events
...
limitReachedAddingRowOrColumn : Config msg -> Flags -> ACrosstab.ErrorAddingRowOrColumn -> XB2.Share.Store.Platform2.Store -> Direction -> Model -> Cmd msg
limitReachedAddingRowOrColumn config flags exceededCounts p2Store direction model =
    Cmd.batch
        [ getAnalyticsCmd flags LimitReachedAddingRowOrColumn { direction = direction } p2Store model
        , Cmd.perform <|
...
limitReachedAddingBase : Config msg -> Flags -> ACrosstab.ErrorAddingBase -> XB2.Share.Store.Platform2.Store -> Model -> Cmd msg
limitReachedAddingBase config flags exceededCounts p2Store model =
    let
        limitReachedMsg =
            config.limitReachedAddingBases
    in
    Cmd.batch
        [ getAnalyticsCmd flags LimitReachedAddingBase {} p2Store model
        , Cmd.perform <|
            limitReachedMsg
                (ACrosstab.getSizeWithTotals <| currentCrosstab model)
                exceededCounts
...
onP2StoreChange : Config msg -> Flags -> XB2.Share.Store.Platform2.Store -> XB2.Share.Store.Platform2.Msg -> Model -> ( Model, Cmd msg )
onP2StoreChange config flags newP2Store p2StoreMsg model =
    case p2StoreMsg of
            ...
            Cmd.pure model
                ...
                |> (\( m, cmds ) ->
                        case m.exportWaitingForQuestions of
                            Just { project, time } ->
                                updateExport config flags project time newP2Store m
                                    |> Cmd.add cmds
        ...
        XB2.Share.Store.Platform2.LocationsByNamespaceFetched _ _ ->
            Cmd.pure model
                |> Glue.updateWith Glue.id (setDefaultLocationsIfNeeded config flags newP2Store { updateCurrentHistory = True })
        ...
        XB2.Share.Store.Platform2.WavesByNamespaceFetched _ _ ->
            Cmd.pure model
                |> Glue.updateWith Glue.id (setFourMostRecentWavesIfNeeded config flags newP2Store { updateCurrentHistory = True })
...
getDatasetCodesFromAudienceExpression : XB2.Share.Store.Platform2.Store -> AudienceExpression -> List XB2.Share.Data.Platform2.DatasetCode
...
trackGroupAddedByAppendToBase :
    ...
    -> Model
    -> Cmd msg
trackGroupAddedByAppendToBase flags counts grouping newExpression p2Store model =
...
trackGroupsAffixed :
    ...
    -> Model
    -> Cmd msg
trackGroupsAffixed flags addedItems groupsToSave grouping operator affixedFrom p2Store model =
    let
...
trackGroupsAddedAsNew :
    ...
    -> Model
    -> Cmd msg
trackGroupsAddedAsNew flags addedItems direction grouping p2Store model =
    let
...
trackItemMoved : Flags -> Direction -> Direction -> XB2.Share.Store.Platform2.Store -> Model -> Cmd msg
trackItemMoved flags from to p2Store model =
    getAnalyticsCmd flags
        ItemMoved
...
trackItemsAdded : Flags -> Grouping -> Destination -> AddedHow -> SelectedItems -> Model -> XB2.Share.Store.Platform2.Store -> Cmd msg
trackItemsAdded flags grouping destination addedHow items model store =
    let
        cellsCount : Int
        cellsCount =
        ...
        eventFromAttribute attribute =
            ...
            ItemAdded
                ...
                , questions = getQuestionsFromCodes <| Expression.questionCodes item.expression
                , datapointCodes = Expression.datapointCodes item.expression
        ...
        eventFromAudience : XB2.Share.Data.Platform2.Audience -> Event
        eventFromAudience audience =
            ItemAdded
                ...
                , cellsCount = cellsCount
                , questions = getQuestionsFromCodes <| Expression.questionCodes audience.expression
                , datapointCodes = Expression.datapointCodes audience.expression
...
trackHeaderCollapsed :
    ...
    -> Model
    -> ( Model, Cmd msg )
trackHeaderCollapsed isCollapsed flags p2Store model =
    let
...
addAudienceItems :
    ...
    -> AudienceItemData
    -> ( Model, Cmd msg )
addAudienceItems config flags grouping direction size p2Store addAudienceToCrosstab model attributeBrowserModal { items, itemsType } =
    let
        audiences :
            List
    ...
    of
        ...
        Err exceededCounts ->
            ( model, limitReachedAddingRowOrColumn config flags exceededCounts p2Store direction model )
...
addAudienceItemsForMerge :
    ...
    -> AudienceItemData
    -> ( Model, Cmd msg )
addAudienceItemsForMerge config flags grouping direction size p2Store addAudienceToCrosstab model attributeBrowserModal asNew allSelected { items, itemsType } =
    let
        audiences :
            List
                { key : { isSelected : Bool, item : Random.Seed -> ( AudienceItem, Random.Seed ) }
...
getAnalyticsCmd :
    ...
    -> Model
    -> Cmd msg
getAnalyticsCmd flags paramsToEvent extraParams p2Store model =
    track flags <| getAnalyticsEvent paramsToEvent extraParams p2Store model
...
resolveAppliedBaseAudience :
    ...
    -> ( Model, Cmd msg )
resolveAppliedBaseAudience config flags p2Store place newBase model maybeResult =
    maybeResult
        |> Maybe.map
            (\result ->
                case result of
                    Ok ( newCrosstab, reloadCellsCommands ) ->
                        ...
                        in
                        newModel
                            |> Cmd.with (getAnalyticsCmd flags BaseAudienceApplied { place = place } p2Store newModel)
                    ...
                    Err exceededCounts ->
                        ...
                        -}
                        ( model, limitReachedAddingBase config flags exceededCounts p2Store model )
            )
...
applyBaseAudiences : Config msg -> Flags -> XB2.Share.Store.Platform2.Store -> Place -> Random.Seed -> NonEmpty BaseAudience -> Model -> ( Model, Cmd msg )
applyBaseAudiences config flags p2Store place seed bases model =
    case
        ACrosstab.addBases
            seed
            bases
        ...
        Ok ( newCrosstab, reloadCellsCommands ) ->
            let
                ...
                lineageRequests =
                    bases
                        |> NonemptyList.toList
                        |> List.fastConcatMap (BaseAudience.getExpression >> Expression.namespaceCodes)
                        |> Set.Any.fromList Namespace.codeToString
                        |> Set.Any.toList
                        |> List.map XB2.Share.Store.Platform2.FetchLineage
            in
...
replaceDefaultBaseAudience :
    ...
    -> Model
    -> ( Model, Cmd msg )
replaceDefaultBaseAudience config flags p2Store newBase place model =
    ACrosstab.replaceDefaultBaseAudience newBase (currentCrosstab model)
        |> Maybe.map Ok
        |> resolveAppliedBaseAudience config flags p2Store place newBase model


removeAudiences : Config msg -> Flags -> XB2.Share.Store.Platform2.Store -> List ( Direction, ACrosstab.Key ) -> Model -> ( Model, Cmd msg )
removeAudiences config flags p2Store items model =
    let
...
applyWavesSelection :
    ...
    -> Model
    -> ( Model, Cmd msg )
applyWavesSelection config flags selectedWaveCodes p2Store { updateCurrentHistory } model =
    let
        ...
        |> Maybe.map
            (\( newCrosstab, reloadCellsCommands ) ->
                ...
                newModel
                    ...
                    |> Cmd.with (getAnalyticsCmd flags WavesChanged {} p2Store newModel)
...
applyLocationsSelection :
    ...
    -> Model
    -> ( Model, Cmd msg )
applyLocationsSelection config flags selectedLocationCodes p2Store { updateCurrentHistory } model =
    let
        closeEverything modelMsg =
            Tuple.mapFirst (\m -> { m | drawer = Drawers.close }) modelMsg
                |> Cmd.addTrigger config.closeModal
    ...
    currentCrosstab model
        ...
        |> Maybe.map
            (\( newCrosstab, reloadCellsCommands ) ->
                ...
                newModel
                    -- TODO: schedule resort after finishing all new requests, OR alternatively reset the sort?
                    |> Cmd.with (getAnalyticsCmd flags LocationsChanged {} p2Store newModel)
                    -- TODO: Investigate this deeply. SetVisibleCellsAndTableOffset cmd allows us to reload the cells already, so do we actually need to call updateCellLoader to fire the commands here?
                    -- TODO: There's a double API trigger here.
                    |> updateCellLoader config (CrosstabCellLoader.interpretCommands config.cellLoaderConfig flags p2Store AudienceIntersect.Table reloadCellsCommands)
            )
...
setFourMostRecentWavesIfNeeded : Config msg -> Flags -> XB2.Share.Store.Platform2.Store -> { updateCurrentHistory : Bool } -> Model -> ( Model, Cmd msg )
setFourMostRecentWavesIfNeeded config flags p2Store updateCurrentHistory model =
    let
        crosstab =
            currentCrosstab model
    in
...
setDefaultLocationsIfNeeded : Config msg -> Flags -> XB2.Share.Store.Platform2.Store -> { updateCurrentHistory : Bool } -> Model -> ( Model, Cmd msg )
setDefaultLocationsIfNeeded config flags p2Store updateCurrentHistory model =
    let
        crosstab =
            currentCrosstab model
    in
...
fetchWavesAndLocations : Config msg -> Flags -> XB2.Share.Store.Platform2.Store -> { updateCurrentHistory : Bool } -> Model -> ( Model, Cmd msg )
fetchWavesAndLocations config flags p2Store updateCurrentHistory model =
    let
        namespaceCodes =
            currentCrosstab model
                |> ACrosstab.namespaceCodes
    in
    Cmd.pure model
        |> Glue.updateWith Glue.id (setFourMostRecentWavesIfNeeded config flags p2Store updateCurrentHistory)
        |> Glue.updateWith Glue.id (setDefaultLocationsIfNeeded config flags p2Store updateCurrentHistory)
        |> Cmd.addTrigger
            (config.fetchManyP2
...
updateEdit :
    ...
    -> Model
    -> ( Model, Cmd msg )
updateEdit config flags xbStore p2Store editMsg model =
    case editMsg of
        Move { to, at, items } ->
            let
                sort : Sort
            ...
            case moveFunction at items (currentCrosstab model) of
                Ok ( newCrosstab, reloadCellsCommands ) ->
                    ...
                    , trackItemMoved flags Row to p2Store newModel
                    )
                ...
                Err exceededCounts ->
                    ( model, limitReachedAddingRowOrColumn config flags exceededCounts p2Store to model )
        ...
        MergeRowOrColumn grouping items directions asNew allSelected ->
            let
                ...
                constructTheMergeWith :
                    (NonEmpty AudienceExpression -> AudienceExpression)
                    -> ( Model, Cmd msg )
                ...
                (\{ baseAudience, newCaption, newExpression, grouping, addedItems } ( model_, cmds ) ->
                    ...
                    of
                        Just ( newCrosstab, reloadCellsCommands ) ->
                            ...
                            in
                            ( newModel, cmds )
                                |> Cmd.add (trackGroupAddedByAppendToBase flags counts grouping newExpression p2Store model)
                                |> updateCellLoader config
                                    (CrosstabCellLoader.interpretCommands config.cellLoaderConfig flags p2Store AudienceIntersect.Table reloadCellsCommands)
        ...
        ApplyLocationsSelection selectedLocationCodes _ ->
            applyLocationsSelection config flags selectedLocationCodes p2Store { updateCurrentHistory = False } model

        ApplyWavesSelection selectedWaveCodes ->
            applyWavesSelection config flags selectedWaveCodes p2Store { updateCurrentHistory = False } model
        ...
        RemoveSelectedAudiences doNotShowAgain allSelected ->
            ...
            in
            model
                |> removeAudiences config flags p2Store allSelected
                |> Glue.updateWith Glue.id
            ...
            let
                ...
                index =
                    XB2.Share.UndoRedo.current model.crosstabData
                        ...
                        |> Maybe.withDefault 0
            in
            addAudienceItems config flags Split direction 1 p2Store (addAudiencesAtIndex direction <| index + 1) model Nothing duplicatedItem
                |> Cmd.add
                    (getAnalyticsCmd flags
        ...
        RemoveAudience toRemove ->
            ...
            in
            model
                |> removeAudiences config flags p2Store [ toRemove ]
                |> Tuple.mapFirst possiblyResetSort
        ...
        AddFromAttributeBrowser direction grouping attributeBrowser addedItems ->
            ...
            itemsFromAttributeBrowser grouping addedItems
                |> Maybe.unwrap (Cmd.pure model)
                    (addAudienceItems config flags grouping direction size p2Store (addAudiences direction) model attributeBrowser
                        >> Tuple.mapFirst (updateCrosstabData (refreshOrderBeforeSorting axis))
                    )
                |> Cmd.add analyticsAddedAsNew
                |> Cmd.add (trackItemsAdded flags grouping (directionToDestination direction) AddedAsNew addedItems model p2Store)
                |> Cmd.addTrigger (config.fetchManyP2 lineageRequests)
                |> Glue.updateWith Glue.id (fetchWavesAndLocations config flags p2Store { updateCurrentHistory = True })
        ...
        SetGroupTitle direction { oldKey, newItem, expression } ->
            let
                datapointsCount =
                    expression
                        |> Maybe.map (Expression.foldr ((+) << List.length << .datapointCodes) 0)
            ...
            let
                ...
                handleCommands { commands, counts } crosstab =
                    ...
                    , Cmd.pure model
                        |> updateCellLoader config
                            (CrosstabCellLoader.interpretCommands config.cellLoaderConfig flags p2Store AudienceIntersect.Table commands)
                        |> Cmd.add (trackGroupsAffixed flags addedItems groupsToSave grouping operator affixedFrom p2Store model)
                        |> Cmd.add (trackItemsAdded flags grouping destination AddedByAppend addedItems model p2Store)
                        |> Cmd.addTrigger config.closeModal
        ...
        SwitchCrosstab ->
            let
                ...
                handleCommands =
                    Cmd.add (getAnalyticsCmd flags Flipped {} p2Store model)
            ...
            let
                ...
                analyticsCmd =
                    ...
                    XB2.Share.UndoRedo.currentTag model.crosstabData
                        |> Maybe.map (\event -> getAnalyticsCmd flags Analytics.UndoApplied { undoEvent = event } p2Store model)
                        |> Maybe.withDefault Cmd.none
            in
            ...
            let
                ...
                analyticsCmd =
                    ...
                    XB2.Share.UndoRedo.currentTag newData
                        |> Maybe.map (\event -> getAnalyticsCmd flags Analytics.RedoApplied { undoEvent = event } p2Store model)
                        |> Maybe.withDefault Cmd.none
            in
        ...
        ApplyMetricsSelection metrics ->
            model
                ...
                |> Cmd.withTrigger config.closeModal
                |> Cmd.add (getAnalyticsCmd flags MetricsChosen { metrics = metrics } p2Store model)
...
updateSelectAction : Config msg -> Flags -> TableSelectMsg -> Model -> ( Model, Cmd msg )
updateSelectAction config flags selectActionType model =
    let
    ...
    case selectActionType of
            ...
            executeIfMouseDownMovedWhileDown model
                (\() ->
                    model
                        ...
                        |> Cmd.with (maybeTrack flags <| keyToItemSelectedEvent Row itemSelected key)
                        |> Cmd.add
                            (Task.attempt
                                (always <| config.msg NoOp)
                                (Dom.focus elementToFocus)
                            )
                        |> Cmd.addIf shiftPressed (maybeTrack flags <| keyToItemSelectedWithShiftEvent Row key)
                )
            ...
            executeIfMouseDownMovedWhileDown model
                (\() ->
                    model
                        ...
                        |> Cmd.with (maybeTrack flags <| keyToItemSelectedEvent Column itemSelected key)
                        |> Cmd.add
                            (Task.attempt
                                (always <| config.msg NoOp)
                                (Dom.focus elementToFocus)
                            )
                        |> Cmd.addIf shiftPressed (maybeTrack flags <| keyToItemSelectedWithShiftEvent Column key)
                )
        ...
        SelectAllRows ->
            ...
            model
                ...
                |> Cmd.with (track flags <| keysToItemsSelectedEvent nonselectedRows)
        ...
        SelectAllColumns ->
            ...
            model
                ...
                |> Cmd.with (track flags <| keysToItemsSelectedEvent nonselectedColumns)
...
createBaseFromItems : Config msg -> Flags -> Grouping -> XB2.Share.Store.Platform2.Store -> List ACrosstab.Key -> Model -> ( Model, Cmd msg )
createBaseFromItems config flags grouping p2Store items model =
    let
        constructWith : (NonEmpty AudienceExpression -> AudienceExpression) -> ( Model, Cmd msg )
        constructWith combineExpressions =
            items
                |> List.filterMap
                    (\key ->
                ...
                |> Maybe.map (\( combinedItem, _ ) -> addNewBases config flags p2Store [ ACrosstab.Key combinedItem False ] model)
                |> Maybe.withDefault (Cmd.pure model)
    in
    case grouping of
        Split ->
            addNewBases config flags p2Store items model
...
addNewBases : Config msg -> Flags -> XB2.Share.Store.Platform2.Store -> List ACrosstab.Key -> Model -> ( Model, Cmd msg )
addNewBases config flags p2Store allSelected model =
    let
        withAnalytics =
            Cmd.add (getAnalyticsCmd flags ItemAddedAsABase { rowsColsSelected = List.length allSelected } p2Store model)
...
addNewBase : Config msg -> Flags -> XB2.Share.Store.Platform2.Store -> Model -> ( Model, Cmd msg )
addNewBase config flags p2Store model =
    let
        allSelected =
...
affixModalOpenMsg config grouping { items } caption operator affixedFrom attributeBrowserModal =
    let
        addedItems =
            attributeBrowserModal.browserModel.selectedItems
    in
    if List.isEmpty addedItems then
        ...
        case AudienceItem.getDefinition lastItem of
            ...
            Expression expression ->
                ...
                in
                case attributeBrowserModal.affixingItems of
                    ModalBrowser.NotAffixing ->
                        config.msg NoOp
...
updateExport : Config msg -> Flags -> Maybe XBProject -> Posix -> XB2.Share.Store.Platform2.Store -> Model -> ( Model, Cmd msg )
updateExport config flags maybeProject date p2Store model =
    case
        currentCrosstab model
            |> ACrosstab.questionCodes
            ...
            case exportData of
                ...
                Just data ->
                    let
                        ...
                        analyticsCmd =
                            track flags <| exportEvent locations waves p2Store model project
                    in
                    ( model
                    , Cmd.batch
    ...
    let
        ...
        update_ =
            case msg of
                ...
                UselessCheckboxClicked ->
                    model
                        |> Cmd.with (getAnalyticsCmd flags Analytics.UselessCheckboxClicked {} p2Store model)
                ...
                CellLoaderMsg cellLoaderMsg ->
                    let
                        ...
                        ( newCellLoaderModel, cmds ) =
                            CrosstabCellLoader.update config.cellLoaderConfig flags p2Store cellLoaderMsg currentData.cellLoaderModel
                ...
                Export maybeProject date ->
                    Cmd.pure model
                        ...
                        |> Glue.updateWith Glue.id (updateExport config flags maybeProject date p2Store)
                ...
                NavigateTo route_ ->
                    ( model, Cmd.perform <| config.navigateTo route_ )
                    ...
                    case ( CrosstabCellLoader.isFullyLoaded <| .cellLoaderModel <| currentCrosstabData model, metric ) of
                        ...
                        _ ->
                            let
                                analyticsCmd =
                                    metric
                                        |> Maybe.map (\metric_ -> getAnalyticsCmd flags HeatmapApplied { metric = metric_ } p2Store model)
                                        |> Maybe.withDefault Cmd.none
                            in
                ...
                ToggleHeaderCollapsed ->
                    let
                        newModel =
                            toggleHeaderCollapsed model
                    in
                    trackHeaderCollapsed newModel.isHeaderCollapsed flags p2Store newModel
                    ...
                    let
                        ...
                        trackAnalytics =
                            if affixedFrom /= Analytics.AddAttributeButton then
                                Cmd.add (track flags <| Analytics.AffixAttributesOrAudiences affixedFrom)

                            else
                                Cmd.add (track flags <| Analytics.OpenAttributeBrowser Analytics.OpenForTable)
                ...
                OpenAffixTableModalForSingle item ->
                    ...
                    , Cmd.batch
                        ...
                        , track flags <| Analytics.AffixAttributesOrAudiences Analytics.FromDropDownMenu
                        ]
                    )
                ...
                OpenAttributeBrowserForAddBase ->
                    ...
                    , Cmd.perform <|
                        ...
                        |> Cmd.add (track flags <| Analytics.OpenAttributeBrowser Analytics.OpenForBase)
                                ...
                                |> RemoteData.map
                                    (\settings ->
                                        if XBData.canShow XBData.DeleteRowsColumnsModal settings then
                                            Cmd.batch
                                                [ modalCmd
                                                , getAnalyticsCmd flags RowsColsDeletionModalOpened {} p2Store model
                                                ]
                                ...
                                |> RemoteData.map
                                    (\settings ->
                                        if XBData.canShow XBData.DeleteBasesModal settings then
                                            Cmd.batch
                                                [ modalCmd
                                                , getAnalyticsCmd flags BasesDeletionModalOpened {} p2Store model
                                                ]
                ...
                AddAsNewBase itemToAdd ->
                    addNewBases config flags p2Store [ Tuple.second itemToAdd ] model
                                ...
                                |> List.foldl
                                    (\base ( dp, q ) ->
                                        BaseAudience.getExpression base
                                            |> (\expression ->
                                                    ( Set.Any.union dp
                                                        (Expression.datapointCodes expression
                ...
                AnalyticsEvent event ->
                    Cmd.with (track flags event) model

                RemoveSortingAndCloseModal ->
                    model
                        |> Cmd.withTrigger config.closeModal
                        |> Glue.update Glue.id
                            (update config flags xbStore p2Store)
...
keyToItemSelectedEventData key =
    case AudienceItem.getDefinition key.item of
        Expression expression ->
            Just
                ( XB2.Share.Data.Id.setFromList <| Expression.datapointCodes expression
                , XB2.Share.Data.Id.setFromList <| Expression.questionCodes expression
