...
trackFullLoadAndExport : Config msg -> Flags -> XBProjectFullyLoaded -> XB2.Share.Store.Platform2.Store -> { startTime : Posix, time : Posix } -> Model -> ( Model, Cmd msg )
trackFullLoadAndExport config flags xbProject p2Store times model =
                            ...
                            let
                                tableLoadedEvent =
                                    ...
                                    , extraParams =
                                        ...
                                        |> Analytics.trackEvent flags Place.CrosstabBuilderList
        ...
        XB2.Share.Store.Platform2.QuestionFetched _ _ ->
            case model.exportingProject of
                Just { project, waitingForQuestions } ->
                    model
                        |> Maybe.unwrap Cmd.pure (trackFullLoadAndExport config flags project newP2Store) waitingForQuestions
...
update : Config msg -> Flags -> Zone -> XBStore.Store -> XB2.Share.Store.Platform2.Store -> Msg -> Model -> ( Model, Cmd msg )
update config flags zone xbStore p2Store msg model =
    case msg of
        NoOp ->
            ( model, Cmd.none )
            ...
            case return of
                Just { dropListId, dragItem } ->
                    ...
                    (if XBData.isSharedWithMe dragItem.shared || isDropItemSharedWithMe then
                        ...
                        |> Cmd.add (Analytics.trackEvent flags Place.CrosstabBuilderList Analytics.ManagementPageDragAndDropUsed)
        ...
        CellLoaderMsg cellLoaderMsg ->
            Cmd.pure model
                |> Glue.Lazy.updateWith crosstabCellLoader (CrosstabCellLoader.update config.cellLoaderConfig flags p2Store cellLoaderMsg)
