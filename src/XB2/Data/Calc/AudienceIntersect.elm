...
encodeBase base =
    Encode.object
        ...
        , ( "expression", Expression.encodeV2 <| BaseAudience.getExpression base )
        ]
...
formatValue : IntersectResult -> Metric -> { exactRespondentNumber : Bool } -> String
formatValue result metric { exactRespondentNumber } =
    let
        value : Float
        value =
            getValue metric result
...
encodeAudienceParamForBulk param =
    Encode.object
        ...
        , ( "expression", Expression.encodeV2 param.expression )
        ]
...
type alias BulkAudienceParams =
    { rows : List { id : AudienceItemId, expression : AudienceExpression }
    , columns : List { id : AudienceItemId, expression : AudienceExpression }
