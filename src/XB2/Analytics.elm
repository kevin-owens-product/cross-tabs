...
import XB2.Share.Data.Audience.Expression as Expression exposing (AudienceExpression(..), LogicOperator)
...
type Event
    ...
    | ItemAdded
        ...
        , addedHow : AddedHow
        , itemType : String
        , audienceId : Maybe XB2.Share.Data.Platform2.AudienceId
        , cellsCount : Int
    ...
    | GroupAddedByAffixToBase
        (EventParams
            { groupingOperator : Grouping
            , newExpression : AudienceExpression
            , counts : Counts
            , datasetNames : List String
            }
        )
...
encodeRootLogicOperator : Expression.AudienceExpression -> Encode.Value
encodeRootLogicOperator expr =
    case expr of
        Expression.Node operator _ ->
...
sharedProjectEventParams flags place project store =
    ...
    encodeShared project.data.ownerId project.shared
        ++ [ ( "project_name", Encode.string project.name )
           , ( "project_id", XB2.Share.Data.Id.encode project.id )
...
trackEvent : Flags -> Place -> Event -> Cmd msg
trackEvent flags place event =
    XB2.Share.Analytics.track <| encodeEvent flags place event


trackEvents : Flags -> Place -> List Event -> Cmd msg
trackEvents flags place events =
    XB2.Share.Analytics.batch_ (encodeEvent flags place) events
...
encodeEvent : Flags -> Place -> Event -> ( String, Value )
encodeEvent flags place event =
    case event of
        Flipped eventParams ->
        ...
        ItemAdded { destination, audienceId, itemType, addedHow, cellsCount, questions, datapointCodes, datasetNames, itemLabel } ->
            ...
            , Encode.object
                ...
                , ( "audience_id", Encode.string <| Maybe.unwrap "n/a" XB2.Share.Data.Id.unwrap audienceId )
        ...
        ProjectRenamed { project, store } ->
            ...
            , Encode.object <| sharedProjectEventParams flags place project store
            )
        ...
        ProjectDuplicated { project, originalProject, store } ->
            ...
            , Encode.object <|
                ( "original_project_name", Encode.string originalProject.name )
                    :: ( "original_project_id", XB2.Share.Data.Id.encode originalProject.id )
        ...
        AudienceSaved ({ extraParams } as sharedParams) ->
            let
                datapointCodes =
                    Expression.datapointCodes extraParams.expression
                        |> List.uniqueBy XB2.Share.Data.Id.unwrap

                questionCodes =
                    Expression.questionCodes extraParams.expression
                        |> List.uniqueBy XB2.Share.Data.Id.unwrap
        ...
        OpenAttributeBrowser target ->
            ...
            , Encode.object [ encodeAttributeBrowserOpenType target ]
            )
        ...
        ListSorted { sorting } ->
            ...
            , Encode.object [ ( "sorting", Encode.string sorting ) ]
            )

        TabsClicked { tab } ->
            ( "P2 - Crosstabs - Tab clicked"
            , Encode.object [ ( "tab", Encode.string tab ) ]
            )
        ...
        CopyLink { projectId, projectName } ->
            ...
            , Encode.object
                [ ( "project_id", XB2.Share.Data.Id.encode projectId )
...
analyticsFriendlyExpression : AudienceExpression -> List String
analyticsFriendlyExpression =
    let
        foldlExpression : Int -> LogicOperator -> (Int -> LogicOperator -> Expression.LeafData -> b -> b) -> b -> AudienceExpression -> b
        foldlExpression depth lastLogicOperator f acc expression =
            case expression of
                ...
                Node lo subnodes ->
                    NonemptyList.foldl (\exp listAcc -> foldlExpression (depth + 1) lo f listAcc exp) acc subnodes
        ...
        reducer depth logic leafData =
            (::)
                (String.repeat depth "."
                    ...
                    ++ "qId:"
                    ++ XB2.Share.Data.Id.unwrap leafData.questionCode
                    ++ ",dtps:"
                    ++ commaSeparatedString XB2.Share.Data.Id.unwrap leafData.datapointCodes
                    ++ (if List.isEmpty leafData.suffixCodes then
                            ""

                        else
                            ",sfxs:"
                                ++ commaSeparatedString XB2.Share.Data.Id.unwrap
