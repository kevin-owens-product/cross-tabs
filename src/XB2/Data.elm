...
encodeAudienceDefinition definition =
    case definition of
        Expression expr ->
            ( "expression", Expression.encodeV2 expr )
...
getProjectQuestionCodes project =
    (NonemptyList.toList project.data.bases
        |> List.fastConcatMap (.expression >> Expression.questionCodes)
    )
...
getCrosstabDatasetCodes : List AudienceDefinition -> List AudienceExpression -> Store -> List DatasetCode
...
getProjectDatasetNames : List AudienceDefinition -> List AudienceExpression -> Store -> List String
