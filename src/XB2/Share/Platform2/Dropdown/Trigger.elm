module XB2.Share.Platform2.Dropdown.Trigger exposing
    ( buttonView
    , Required
    , Attribute, class, label, sublabel, leftIcon, disabled
    , RequiredWithBodyFence
    )
...
@docs buttonView
@docs Required
@docs Attribute, class, label, sublabel, leftIcon, disabled
...
sublabel : Maybe String -> Attribute msg
sublabel sublabel_ =
    Attribute (\config -> { config | sublabel = sublabel_ })


leftIcon : Maybe IconData -> Attribute msg
leftIcon leftIcon_ =
    Attribute (\config -> { config | leftIcon = leftIcon_ })
        ...
        [ Html.viewMaybe
            (\leftIcon_ ->
                Html.div
                    ...
                    [ XB2.Share.Icons.icon [] leftIcon_ ]
            )
            config.leftIcon
        , Html.span
