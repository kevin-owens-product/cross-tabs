module XB2.Share.Platform2.Notification.Queue exposing
    ( Msg(..)
    , NotificationData
    , NotificationId(..)
    , NotificationQueue
...
emptyWithLimit : Int -> NotificationQueue msg
emptyWithLimit limit =
    empty_ (Capped limit)
    ...
    let
        ...
        dequeueIfOverLimit ((NotificationQueue data) as queue_) =
            case data.queueType of
                ...
                Capped capSize ->
                    if Queue.size data.notifications > capSize then
                        mapQueue (Queue.dequeue >> Tuple.second) queue_
