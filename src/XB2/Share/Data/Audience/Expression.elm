module XB2.Share.Data.Audience.Expression exposing
    ( AudienceExpression(..)
    , AudienceInclusion(..)
    , LeafData
    , LogicOperator(..)
...
import XB2.Share.Gwi.Json.Decode exposing (stringOrInt)
...
{-| This acts as monoidal composition where identity element is `sizeExpression`.
-}
intersection : AudienceExpression -> AudienceExpression -> AudienceExpression
intersection left right =
    Node And ( left, [ right ] )


intersectionMany : NonEmpty AudienceExpression -> AudienceExpression
intersectionMany =
    Node And


union : AudienceExpression -> AudienceExpression -> AudienceExpression
union left right =
    Node Or ( left, [ right ] )


unionMany : NonEmpty AudienceExpression -> AudienceExpression
unionMany =
    Node Or


append : LogicOperator -> AudienceExpression -> AudienceExpression -> AudienceExpression
append op =
    case op of
        Or ->
            union
    ...
    case expr of
        ...
        Leaf data ->
                ...
                datapointsField =
                    case targetApi of
                        V2 ->
                            "datapoints"
            in
            [ Just ( "question", XB2.Share.Data.Id.encode data.questionCode )
            , Just ( datapointsField, Encode.list XB2.Share.Data.Id.encode data.datapointCodes )
            ...
            , if List.isEmpty data.suffixCodes then
                ...
                Just ( "suffixes", Encode.list XB2.Share.Data.Id.encode data.suffixCodes )
            ]
                |> Maybe.values
                |> Encode.object
...
leafDecoder =
    Decode.succeed LeafData
        ...
        |> Decode.andMap
            ...
            -}
            (Decode.oneOf
                [ Decode.field "datapoints" (Decode.list XB2.Share.Data.Id.decode)
                , Decode.field "options" (Decode.list XB2.Share.Data.Id.decode)
                ]
            )
        |> Decode.andMap
            (Decode.field "suffixes" (Decode.list XB2.Share.Data.Id.decodeFromStringOrInt)
                -- TODO after DB migration change to `XB2.Share.Data.Id.decode`!
                |> Decode.maybe
...
questionCodes : AudienceExpression -> List NamespaceAndQuestionCode
questionCodes expression =
    foldr
        (\leaf acc -> leaf.questionCode :: acc)
        []
        expression


datapointCodes : AudienceExpression -> List QuestionAndDatapointCode
datapointCodes expression =
    foldr
        (\leaf acc -> leaf.datapointCodes ++ acc)
        []
        expression


namespaceCodes : AudienceExpression -> List Namespace.Code
namespaceCodes expr =
    questionCodes expr
        |> List.map XB2.Share.Data.Labels.questionCodeToNamespaceCode
