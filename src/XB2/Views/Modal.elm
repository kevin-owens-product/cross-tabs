...
import XB2.Data.AudienceCrosstab as ACrosstab exposing (AffixGroupItem, Direction(..))
...
type alias AttributesModalData =
    ...
    , affixingItems : ModalBrowser.AffixingItems
    }
...
type alias AffixBaseGroupData =
    ...
    , expressionBeingAffixed : AudienceExpression
    , newCaption : Caption
    , grouping : Grouping
...
type SaveAsItem
    = SaveAsAudienceItem AudienceItem AudienceExpression
...
type alias Config msg =
    ...
    , saveGroupName :
        ...
        ->
            ...
            , expression : Maybe AudienceExpression
            }
        -> msg
    ...
    -- Save as Audience
    , saveAsAudience : SaveAsItem -> Caption -> AudienceExpression -> msg
...
initViewGroup : ( Direction, ACrosstab.Key, AudienceExpression ) -> Modal
...
initAttributesModal : AttributesModalType -> ModalBrowser.AffixingItems -> IdSet WaveCodeTag -> IdSet LocationCodeTag -> Int -> Modal
initAttributesModal modalType affixingItems waves locations selectedBasesCount =
    AttributesModal
        { browserModel = ModalBrowser.init waves locations
        , modalType = modalType
        , selectedBasesCount = selectedBasesCount
        , affixingItems = affixingItems
        }


initAttributesAddModal : IdSet WaveCodeTag -> IdSet LocationCodeTag -> Int -> Modal
initAttributesAddModal =
    initAttributesModal AddRowColumnToTable NotAffixing
...
initAttributesAddBaseModal =
    initAttributesModal AddBaseAudience NotAffixing
...
initAttributesReplaceDefaultBaseModal =
    initAttributesModal ReplaceDefaultBaseAudience NotAffixing
...
update : Config msg -> Flags -> XBStore.Store -> Msg -> Modal -> ( Modal, Cmd msg )
update config flags xbStore msg modal =
    case msg of
        NoOp ->
            Cmd.pure modal
        ...
        KnowledgeBaseLinkClicked name url ->
            modal
                ...
                |> Cmd.add (Analytics.trackEvent flags Place.CrosstabBuilder <| KnowledgeBaseOpened name url)
        ...
        ModalBrowserMsg mbMsg ->
            case modal of
                AttributesModal data ->
                    ModalBrowser.update (config.browser data) xbStore mbMsg data.browserModel
...
removeBulkConfirmContents :
    ...
    ->
        ...
        , items : List ( ExpandedState, { name : String, expression : AudienceExpression } )
        , dialogCopy : Html msg
        , doNotShowAgainChecked : Bool
        }
    -> List (Html msg)
...
affixGroupContentsMany_ :
    ...
    ->
        { newExpression : AudienceExpression
        , currentExpression : AudienceExpression
        , expressionBeingAffixed : AudienceExpression
        , onSubmit : msg
        , headerText : String
        , saveButtonText : String
        , closeModal : msg
        , names : List ( Caption, AudienceExpression )
...
affixGroupContentsMany_ flags data =
    let
        ...
        viewGroup_ : Int -> ( Caption, AudienceExpression ) -> Html msg
