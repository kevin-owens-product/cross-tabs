...
processAfterQueueCmd config model =
    ...
    if not <| ACrosstab.isLoading <| currentCrosstab model then
        { model
            ...
            |> Cmd.with (fullyLoadTableMsg model.afterQueueFinishedCmd)
...
trackNAErrorForCell : Config msg afterAction -> Flags -> CellData -> String -> XB2.Share.Store.Platform2.Store -> Model afterAction -> ( Model afterAction, Cmd msg )
trackNAErrorForCell config flags cellData key store model =
    let
        trackFailure result =
            case result of
                Failure error ->
                    ...
                    , NAIntersection
                        ...
                        |> Analytics.trackEvent flags config.analyticsPlace
                    )
...
update :
    ...
    -> Model afterAction
    -> ( Model afterAction, Cmd msg )
update config flags p2Store msg model =
    let
    ...
    case msg of
        ...
        CellLoaded locationCodes waveIds key value ->
            let
                existingCell : Cell
                existingCell =
            ...
            model
                |> insertCell
                |> Glue.updateWith Glue.id (trackNAErrorForCell config flags value (cellToRetryKey key) p2Store)
                |> Glue.updateWith Glue.id
                    (dequeueAndInterpretCommand config
                        flags
                        p2Store
        ...
        BulkCellsLoaded locationCodes waveIds baseAudience crosstabTableCells crosstabTotalCells ->
        ...
        TotalsCellLoaded locationCodes waveIds { col, row } { item, base } value ->
            let
                key_ : ( AudienceItem, BaseAudience )
                key_ =
                    ( item, base )
            ...
            model
                |> insertCell
                |> Glue.updateWith Glue.id (trackNAErrorForCell config flags value (totalCellToRetryKey key_) p2Store)
                |> Glue.updateWith Glue.id
                    (dequeueAndInterpretCommand config
                        flags
                        p2Store
        ...
        BulkCellsLoadError err trackerId locationCodes waveIds baseAudience crosstabTableCells crosstabTotalCells ->
...
type alias BulkAvARequestData =
    { rows : List { id : AudienceItemId.AudienceItemId, expression : AudienceExpression, key : ACrosstab.Key }
    , cols : List { id : AudienceItemId.AudienceItemId, expression : AudienceExpression, key : ACrosstab.Key }
...
type alias CrosstabBulkAvARequestData =
    { rows : List ACrosstab.Key
    , cols : List ACrosstab.Key
    , rowExprs : List AudienceExpression
    , colExprs : List AudienceExpression
    }
    ...
    let
        ...
        audienceItemToQueryAudienceParam :
            ...
            ->
                ...
                , expression : Expression.AudienceExpression
                , key : ACrosstab.Key
                }
...
type alias AverageRequestData =
    ...
    , audience : Maybe AudienceExpression
        ...
        getP2AttributesFromExpression =
            Expression.foldr
                (\leaf acc ->
                    let
                        ( namespace, qCode ) =
                            XB2.Share.Data.Labels.splitQuestionCode leaf.questionCode

                        maybeFirstSuffixCode : Maybe SuffixCode
                        maybeFirstSuffixCode =
                            List.head leaf.suffixCodes
                    in
                    List.map
                        (\dtpCode ->
                            let
...
sendAverageRowRequest p2Store data_ { config, flags, trackerId, activeWaves, activeLocations, baseAudience } =
...
sendAverageColRequest p2Store data_ { config, flags, trackerId, activeWaves, activeLocations, baseAudience } =
    resolveAverageRequest config
        data_.average
        data_.getData
        p2Store
...
sendTotalRowAverageColRequest p2Store data { config, flags, trackerId, activeWaves, activeLocations, baseAudience } =
    resolveAverageRequest config
        data.average
        data.getData
        p2Store
...
sendTotalColAverageRowRequest p2Store data { config, flags, trackerId, activeWaves, activeLocations, baseAudience } =
    resolveAverageRequest config
        data.average
        data.getData
        p2Store
