module XB2.Share.Gwi.List exposing
    ( addIf
    , combineRemoteData
    , fastConcat
    , fastConcatMap
...
reverseSortWith compareFn =
    List.sortWith compareFn >> List.reverse
...
{-| A more intuitive variant of List.Extra.gatherEqualsBy
-}
groupBy : (a -> b) -> List a -> List { key : b, items : ( a, List a ) }
groupBy fn list =
    List.gatherEqualsBy fn list
        |> List.map
            (\( first, rest ) ->
                { key = fn first
                , items = ( first, rest )
