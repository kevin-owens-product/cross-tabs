...
type alias Config item msg =
    ...
    , closeDropdown : msg
    , getItemIsExcluded : item -> Maybe Bool
    , isADropdownOpen : Bool
    }
            ...
            case config.getItemChildren item_.item of
                ...
                Nothing ->
                    let
                        ...
                        inclusionLabelView =
                            if config.getItemIsExcluded item_.item == Just True then
                                Html.span
                                    [ WeakCss.nestMany
                                        [ "item-group"
                                        , "item-row"
                                    ...
                                    [ Html.span
                                        [ WeakCss.addMany
                                            ...
                                            moduleClass
                                            |> WeakCss.withActiveStates [ "excluded" ]
                                        ]
                                        [ Html.text "Exclude" ]
                                    ]

                            else if config.getItemIsExcluded item_.item == Just False then
                                Html.span
                                    [ WeakCss.nestMany
                                        [ "item-group"
                                        , "item-row"
                    ...
                    Html.div [ WeakCss.nestMany [ "item-group", "item-row" ] moduleClass ]
                        [ Html.div
                                    ...
                                    case groupWithState of
                                        Just state ->
                                            if state.canGrouping then
                                                iconForGroupingState { state | isActive = state.isActive || isThisGroupActive }
                            ...
                            , Html.div [ WeakCss.nestMany [ "item-group", "item-row", "item", "names" ] moduleClass ]
                                [ item_.subtitle
                                    |> Html.viewMaybe
                                        (\subt ->
                                            Html.div
                                                [ WeakCss.nestMany [ "item-group", "item-row", "item", "names", "subtitle" ] moduleClass ]
                                                [ Html.text subt ]
                                        )
                                , Html.div
                                    [ WeakCss.addMany [ "item-group", "item-row", "item", "names", "title" ] moduleClass
                                        |> WeakCss.withStates [ ( "subtitle-only", subtitle == Nothing ) ]
                                    ]
                                    [ let
                                        finalTitle =
                                            if config.getItemIsExcluded item_.item == Just True then
                                                if String.left 4 item_.title == "NOT " then
                                                    String.dropLeft 4 item_.title
        ...
        Nothing ->
            ...
            let
                ...
                inclusionLabelView =
                    case type_ of
                        Attribute ->
                            if config.getItemIsExcluded item == Just True then
                                Html.span
                                    [ WeakCss.nestMany [ "inclusion-label" ]
                                        moduleClass
                                    ]
                                    [ Html.span
                                        [ WeakCss.addMany [ "inclusion-label", "text" ]
                                            moduleClass
                                            |> WeakCss.withActiveStates [ "excluded" ]
                                        ]
                                        [ Html.text "Exclude" ]
                            ...
                            else if config.getItemIsExcluded item == Just False then
                                Html.span
                                    [ WeakCss.nestMany [ "inclusion-label" ]
                                        moduleClass
                                    ]
                                    [ Html.span
                                        [ WeakCss.nestMany [ "inclusion-label", "text" ]
                                            moduleClass
                                        ]
                                        [ Html.text "Include" ]
                                    ]
    ...
    Html.div
        ...
        [ Html.div
            ...
            , Html.button
                ...
                , Attrs.title "Undo (CTRL / ⌘ Z)"
                , Events.onClick config.undo
                , Attrs.disabled (not config.canUndo)
                , WeakCss.nest "undo-btn" moduleClass
                ]
                [ XB2.Share.Icons.icon [ XB2.Share.Icons.width 32 ] P2Icons.undo ]
            , Html.button
                [ Attrs.attribute "aria-label" "Redo"
                , Attrs.title "Redo (CTRL Y / Shift ⌘ Z)"
                , Events.onClick config.redo
                , Attrs.disabled (not config.canRedo)
                , WeakCss.nest "redo-btn" moduleClass
                ]
