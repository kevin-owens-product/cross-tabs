...
fromDatapoint : { question : String, datapoint : Maybe String, suffix : Maybe String, isExcluded : Bool } -> Caption
fromDatapoint { question, datapoint, suffix, isExcluded } =
    let
        name =
            case datapoint of
                Just d ->
                    if isExcluded then
                        "NOT " ++ d ++ (Maybe.map (\suf -> ": " ++ suf) suffix |> Maybe.withDefault "")
